...
- **Launch via logi-ffb.** Launch options `logi-launch %command%` (it keeps raw HID off and runs the logi-ffb helper for you); Steam Input off. Simulated TrueForce needs the community rF2SharedMemoryMapPlugin plus logi-tf-relay in the prefix (see docs/SHARED_MEMORY_RELAY.md).
