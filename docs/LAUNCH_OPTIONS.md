...
| `LOGI_STEER_LOCK` | `1` | The escape proxy applies the steering lock the game sets. Assetto Corsa EVO sends it as a DirectInput escape rather than through the SDK, which Wine drops; on Windows G HUB acts on it. The proxy writes it to the wheel's range and tells the SDK, so the game's lock moves the wheel here too (#91). `0` leaves the lock to the driver's own range |
