...
| `PROTON_ENABLE_HIDRAW=0x046D/0xC276 %command%` | Lets the game talk to the wheel's raw HID interface, which is how Logitech's SDK delivers TrueForce | Set by `logi-launch`, scoped to your wheel. By hand: a **direct-drive wheel** (RS50, G PRO) in a game with its own TrueForce (ACC, Assetto Corsa EVO), with the TrueForce files installed. Use your own product id, not `0xC276`. One known cost: on one CachyOS machine every Proton build tried (Valve's hotfix branch, GE-Proton 11) paid a constant frame-time penalty in AC EVO with the wheel exposed this way, moving or not, while the same wheel on `proton-cachyos` here does not; the cost is in Proton's raw HID layer, and empty launch options give full force feedback without TrueForce (issue #74) |
...
| `LOGI_RANGE_PUSH` | `1` | The escape proxy tells Logitech's SDK the wheel's real rotation range once, right after the game selects the force mode. On the G923 Xbox edition the SDK renders the endstops itself at the edges of the range it believes, and it never learns the real one on that wheel, so without this the rim hits a wall of full torque a few tens of degrees off centre and the game's force is lost under it (#91). `0` leaves the SDK's own belief alone |
| `LOGI_STEER_LOCK` | `1` | The escape proxy applies to the wheel the range Logitech's SDK announces. Whenever the SDK's operating range is set, it tells the driver through a DirectInput escape; on Windows that is how the wheel follows a game's steering lock, under Wine the escape is dropped. The proxy writes that range to the wheel's sysfs range instead (#91). `0` leaves the wheel's range to the driver alone |
...
| `tfroute` | `capture`, `sdk` | how a game's own TrueForce reaches a direct-drive wheel. `sdk` (the default) lets Logitech's library drive the wheel through raw HID. `capture` keeps raw HID off and carries the same TrueForce another way: the SDK proxy (`logi-shim --proxy`, required in the game's prefix) copies what the game hands the library to `logi-tf-sim`, which streams it natively, with the driver's force merged in, and synthesises nothing. It exists because the library's 1,000 packets a second, each answered by the wheel, all cross Wine's device process and wineserver, and on some setups that stalls the game (#74, #121). Setting it also sets `hidraw=0`, `texture=none`, `tfsim=1` and the game's relay for the rev lights. Experimental: feel it against the default before relying on it |
