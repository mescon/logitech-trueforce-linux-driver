...
| `tfsim` | `1`, `0` | run `logi-tf-sim`. Set `0` for a game whose own TrueForce already reaches your wheel. Asking for it alongside `texture=merge` is a combination only a hand-written line can produce, and the two then compete for the one telemetry port: `logi-rpm-bridge` holds it, the daemon runs without its relay games, and the log says so before either complains on its own |
