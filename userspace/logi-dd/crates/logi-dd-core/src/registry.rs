...
pub const REGISTRY: &[SettingSpec] = &[
    ...
    SettingSpec { attr: "wheel_brake_force", label: "Brake force", help: "How hard you must press the load-cell brake to reach full braking. Lower it for a lighter pedal, raise it if you bottom out too easily (0-100%). The wheel only stores this in onboard mode; in desktop mode use Brake sensitivity or the brake curve instead.", category: Pedals, kind: PCT, access: ReadWrite, mode_req: OnboardOnly },
